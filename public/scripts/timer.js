...
(function(root, factory) {
	...
	var intr,
		...
		options = {
			seconds: 0,	
			minutes: 0,								// default seconds value to start timer from
	...
	var Timer = function(element, userOptions) {
		...
		$(element).data('totalMinutes', options.minutes);

		// Setup start time if seconds were provided
		$(element).data('startTime', getUnixSeconds() - $(element).data('totalSeconds'));
		$(element).data('startTime', getUnixMinutes() - $(element).data('totalMinutes'));

		$(element).data('seconds', $(element).data('totalSeconds'));
		$(element).data('minutes', $(element).data('totalMinutes'));
		$(element).data('state', TIMER_STOPPED);
